...
//	       jQuery("A[name=save]", $dialog).button({icons: { primary: "ui-icon-check" }});
	       ...
	       jQuery("BUTTON[name=publishbook]", $dialog).button({icons: { primary: "ui-icon-closethick" }}).click(function() {
		   var publishMode = jQuery("FORM[name=formexport] INPUT[name=publish]:checked", $dialog).val();
		   var commands = {"command": "publish_book2",
				   "publish_mode": publishMode};

		   jQuery('a[name=publishbook]').button("option", "disabled", true);
		   jQuery(".downloadlink", $dialog).html("Please wait...");
		   ...
		   booktypeNetwork.sendToCurrentBook(commands,						  
							 ...
							 if(typeof data.status == 'undefined' || data.status == false) {
							     jQuery(".downloadlink", $dialog).html('There has been an error while trying to publish book.');
							 } else {
							     jQuery(".downloadlink", $dialog).html('<a target="_new" href="'+data.dta+'">'+data.dta+'</a>');
							 }
	   ...
	   function _openTOC(showClose) {
		   ...
		   jQuery(".infomessage", $panel).html('<span class="alert-box info">Loading chapters</span>');
		       ...
		       jQuery("A[name=cancel]", $panel).show();
		   } else {
		       if(showClose) {
			   jQuery("A[name=cancel]", $panel).show();
		       } else {
			   jQuery("A[name=cancel]", $panel).hide();
		       }
		   }
	   ...
	   function _initEditor() {
	       ...
	       // This is not good. There has to be a way how to get prefix for link.
	       jQuery('.topmenu .userinfo').html(BOOKTYPE.username);		   

//	       jQuery('.topmenu .userinfo').html('<a href="/accounts/'+BOOKTYPE.username+'/">'+BOOKTYPE.username+'</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="/accounts/signout/">Sign out</a>&nbsp;&nbsp;|&nbsp;&nbsp;'+jQuery("#invisible-message").html());		   
